...
class ModifiedRotatedPlanarRMPSDecoder(Decoder):
    r"""
    NOTE:

    This class is a modified version of `RotatedPlanarRMPSDecoder` from the
    [qecsim](https://github.com/qecsim/qecsim) library by James Roffe and contributors.

    Original implementation: qecsim.decoders.planar.RotatedPlanarRMPSDecoder
    Modifications in this version: Added the ability to permute the Pauli noise channels on a per data-qubit basis.

    The lines that has been changed or added are maked with `--- Modified ---` comments.
    ...
    * Contracting by columns (i.e. truncating vertical links) may give different coset probabilities to contracting by
    ...
    def _coset_probabilities(self, prob_dist, sample_pauli, error_permutation): # --- Modified ---
        r"""
        ...
        tns = [self._tnc.create_tn(prob_dist, sp, error_permutation) for sp in sample_paulis] # --- Modified ---
        # probabilities
    ...
    def decode(self, code, syndrome,
               ...
               error_permutation=None, # --- Modified ---
               **kwargs):
        """
        ...
        :param error_permutation: Optional permutation of error channels on a per qubit basis. (default=None, no permutation)
        :type error_permutation: numpy.array of shape (L, L, 4) or None
        ...
        coset_ps, recoveries = self._coset_probabilities(prob_dist, any_recovery, error_permutation) # --- Modified ---
    ...
    class TNC:
        ...
        def create_tn(self, prob_dist, sample_pauli, error_permutation): # --- Modified ---
            ...
            for code_index in itertools.product(range(max_site_x + 1), range(max_site_y + 1)):
                ...
                # create q-node
                permuted_prob_dist = prob_dist if error_permutation is None else tuple(prob_dist[i] for i in error_permutation[*code_index]) # --- Modified ---
                q_node = self.create_q_node(permuted_prob_dist, q_pauli, is_h_node, is_even_column, q_direction) # --- Modified ---
